...
func GetpodcastHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetpodcastTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v2_podcasts_podcastKey",
		mcp.WithDescription("Get podcast"),
		mcp.WithString("podcastKey", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetpodcastHandler(cfg),
	}
}
